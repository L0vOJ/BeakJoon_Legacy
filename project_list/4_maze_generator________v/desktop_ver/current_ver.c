...
#define len 37  //무조건 홀수여야만 한다  4의배수 +1의 값만 가능하다
...
int main(){
  ...
  while(1){
    ...
    printf("미로를 저장하고 싶다면 5번을, 미로를 불러오고싶다면 6번을 입력하세요\n");
    scanf("%d",&want);
...
block** operate(int want,block**maze){
  switch(want){
    ...
    case 5:{
      FILE *fptr;
      char filename[51];
      printf("생성할 미로 파일의 이름을 적으시오(50자 이내):");
      scanf("%s",filename);
      fptr = fopen(filename,"wb");
      fwrite(maze,sizeof(block),len*len,fptr);
      fclose(fptr);
      printf("저장 완료\n");
    }break;
    case 6:{
      ...
      char filename[51];
      while(1){
        printf("불러올 미로 파일의 이름을 적으시오:");
        scanf("%s",filename);
        fptr = fopen(filename,"rb");
        if(fptr == NULL){
          printf("불러오는데 실패했습니다. 다시 입력해주십시오\n");
          continue;
        }
        break;
      }
      fread(buffer,sizeof(block),len*len,fptr);
      printf("%d",buffer[0][0].wall);
      wall_check(buffer);
      /*
      typedef struct{
        int x;
      ...
      */
      ...
      for(int a=0;a<len;a++){
        for(int b=0;b<len;b++){
          ...
          me.y=buffer[a][b].y;
          printf("b");
          me.wall=buffer[a][b].wall;
          printf("c");
          me.root=buffer[a][b].root;
          printf("d");
          me.path=buffer[a][b].path;
          printf("e");
          me.cline=buffer[a][b].cline;
          printf("f");
        }
        printf("%d",pp++);
      }
