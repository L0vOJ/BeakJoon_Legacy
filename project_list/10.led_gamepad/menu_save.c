...
void draw_map(segment* flag, box** map){
  for(int i=0;i<3;i++){
    for(int b=0;b<seglen;b++){
      if(flag->light[i]){
        map[flag->y+(i*(seglen+1))][flag->x+b+1].wall=1;
      }
    }
  }
  for(int i=0;i<2;i++){
    for(int j=0;j<2;j++){
      for(int b=0;b<seglen;b++){
        if(flag->light[3+j+(2*i)]){
          map[flag->y+b+1+(i*(seglen+1))][flag->x+(j*(seglen+1))].wall=1;
        }
      }
    }
  }
