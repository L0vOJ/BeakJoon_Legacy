...
#define len 33  //무조건 홀수여야만 한다  4의배수 +1의 값만 가능하다
...
route_sequence* trevel_route(int a, int b, block**maze, route_sequence* stack, int back);
route_sequence* route_numbering(route_sequence* stack,int a);
route_sequence* draw_route(block**maze);
void route_order(route_sequence* stack);
//미로 초기화 및 생성 함수
...
block** operate(int want,block**maze){
  switch(want){
    ...
    case 4:{
      ...
      route_sequence* stack;
      stack = draw_route(maze);
      stack->direction[stack->count-1]=0;
      route_order(stack);
    }break;
  }
  return maze;
...
route_sequence* route_numbering(route_sequence* stack,int a){
  stack->count++;
  //stack->direction = realloc(stack,sizeof(int)*stack->count);
  stack->direction[stack->count-2]=a;
  printf("(%d,%d)",stack->count-2,stack->direction[stack->count-2]);
  return stack;
}

route_sequence* draw_route(block**maze){
  route_sequence* stack;
  //stack->direction = malloc(4);
  stack->count=1;
  for(int a=2;a<len-1;a+=2){
    stack = route_numbering(stack,2);
    if(maze[1][a].wall==1){
      maze[1][a].root=1;
      system("cls");
      root_check(maze);
      ...
      root_check(maze);
      printf("(%d,%d)",stack->count-2,stack->direction[stack->count-2]);
      stack = route_numbering(stack,3); //down 명령
      stack = trevel_route(2,a,maze,stack,1); //up 회수
    }
  }
  stack = route_numbering(stack,2);
  for(int a=2;a<len-1;a+=2){
    stack = route_numbering(stack,3);
    if(maze[a][len-2].wall==1){
      ...
      root_check(maze);
      printf("(%d,%d)",stack->count-2,stack->direction[stack->count-2]);
      stack = route_numbering(stack,4);
      stack = trevel_route(a,len-3,maze,stack,2);
    }
  }
  stack = route_numbering(stack,5);
  stack = route_numbering(stack,3);
  stack = route_numbering(stack,6);
  for(int a=len-3;a>1;a-=2){
    ...
    if(maze[len-2][a].wall==1){
      ...
      root_check(maze);
      printf("(%d,%d)",stack->count-2,stack->direction[stack->count-2]);
      stack = route_numbering(stack,1);
      stack = trevel_route(len-3,a,maze,stack,3);
    }
  }
  stack = route_numbering(stack,4);
  for(int a=len-3;a>1;a-=2){
    ...
    if(maze[a][1].wall==1){
      ...
      printf("(%d,%d)",stack->count-2,stack->direction[stack->count-2]);
      stack = route_numbering(stack,2);
      stack = trevel_route(a,2,maze,stack,4);
    }
  }
  return stack;
}
...
route_sequence* trevel_route(int a,int b,block**maze,route_sequence* stack, int back){
  if(up.wall==1){
    if(maze[a-2][b].root==0){
      ...
      root_check(maze);
      printf("(%d,%d)",stack->count-2,stack->direction[stack->count-2]);
      stack = route_numbering(stack,1); //up명령
      stack = trevel_route(a-2,b,maze,stack,3); //down회수
    }
  }
  if(right.wall==1){
    if(maze[a][b+2].root==0){
      ...
      printf("(%d,%d)",stack->count-2,stack->direction[stack->count-2]);
      stack = route_numbering(stack,2);
      stack = trevel_route(a,b+2,maze,stack,4);
    }
  }
  if(down.wall==1){
    if(maze[a+2][b].root==0){
      ...
      printf("(%d,%d)",stack->count-2,stack->direction[stack->count-2]);
      stack = route_numbering(stack,3);
      stack = trevel_route(a+2,b,maze,stack,1);
    }
  }
  if(left.wall==1){
    if(maze[a][b-2].root==0){
      ...
      printf("(%d,%d)",stack->count-2,stack->direction[stack->count-2]);
      stack = route_numbering(stack,4);
      stack = trevel_route(a,b-2,maze,stack,2);
    }
  }
  stack = route_numbering(stack,back);
  return stack;
}

void route_order(route_sequence* stack){
  int a=0;
  while(stack->direction[a]){
    switch(stack->direction[a]){
      case 1:{
        printf("up()/");
      }break;
      case 2:{
